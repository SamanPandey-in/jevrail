...
> **Status: MVP implemented.** `jevrail hook`, `explain`, `install`/`uninstall`, `doctor`, `log`, `eval`, and `exec` all work. The policy thresholds are still starting guesses: see [Evaluation](docs/BENCHMARK.md). Design is in [`docs/plan.md`](docs/plan.md); deviations are in [`docs/DEVIATIONS.md`](docs/DEVIATIONS.md). [Demo view](https://drive.google.com/file/d/1ksJ9aENsfMtx9wtLVekHFqu3kjrnCAhj/view?usp=sharing)
...
The README covers install, day-to-day commands, and configuration. Everything about *how* jevrail decides, *what* it sends over the network, and where it's headed lives in [`docs/`](docs/):

| Doc | Covers |
|---|---|
| [docs/HOW_IT_WORKS.md](docs/HOW_IT_WORKS.md) | Why jevrail exists, the pipeline, the "model can only tighten" rule |
| [docs/WHAT_IT_CHECKS.md](docs/WHAT_IT_CHECKS.md) | The 7 questions asked per command, and the context sent with them |
| [docs/DECISIONS.md](docs/DECISIONS.md) | How probabilities become allow/ask/deny, default bands, degraded fallback |
| [docs/USAGE.md](docs/USAGE.md) | Full worked examples for every command |
| [docs/SUPPORTED_AGENTS.md](docs/SUPPORTED_AGENTS.md) | Per-agent implementation status, including a known opencode subagent gap |
| [docs/PRIVACY.md](docs/PRIVACY.md) | Exactly what does and doesn't leave your machine |
| [docs/BENCHMARK.md](docs/BENCHMARK.md) | Evaluation harness, current numbers, corpus |
| [docs/ROADMAP.md](docs/ROADMAP.md) | What's shipped vs. planned |
| [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) | Building, testing, project layout |
| [docs/DEVIATIONS.md](docs/DEVIATIONS.md) | Where the shipped MVP differs from [docs/plan.md](docs/plan.md) |
...
See [docs/SUPPORTED_AGENTS.md](docs/SUPPORTED_AGENTS.md) for per-agent status and [docs/USAGE.md](docs/USAGE.md#3-jevrail-install--uninstall--hook-wiring) for the full `install`/`uninstall` walkthrough.
...
Full worked examples, sample output, and flags for every command above: [docs/USAGE.md](docs/USAGE.md).

---

## Configuration

`~/.config/jevrail/config.json` (JSON today; plan sketches TOML: see [docs/DEVIATIONS.md](docs/DEVIATIONS.md)). Only this file changes if TOML is added later.
...
- `no_model=true` → deterministic-only, nothing leaves the machine. Full data-flow details: [docs/PRIVACY.md](docs/PRIVACY.md).
...
- `bands`: per-harm overrides merged over `policy.DefaultBands`. Default values and how bands turn into a verdict: [docs/DECISIONS.md](docs/DECISIONS.md).
...
- **False asks are a real cost.** 57.7% false-ask on safe in `--no-model` degraded mode (see [docs/BENCHMARK.md](docs/BENCHMARK.md)) is conservative by design; with a real model this drops. If jevrail nags too often, people uninstall it: tune bands per project.
...
- **Per-agent gaps.** Codex's schema is unverified; opencode's subagents may currently bypass the hook entirely. See [docs/SUPPORTED_AGENTS.md](docs/SUPPORTED_AGENTS.md).
...
Building and testing from source: [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md). Where things are headed: [docs/ROADMAP.md](docs/ROADMAP.md).
